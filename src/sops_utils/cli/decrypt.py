...
from sops_utils.core import decrypt_file, encrypt_file, find_env_files
from sops_utils.kubernetes import load_service_definitions, write_service_manifests


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decrypt env files with SOPS.")
    parser.add_argument(
    ...
    if args.age_key_file is not None and not args.age_key_file.is_absolute():
        args.age_key_file = args.root_dir / args.age_key_file
    return args
...
def main() -> int:
    ...
    if shutil.which("sops") is None:
        ...
        return 1
    source_files = find_env_files(args.root_dir, encrypted=True)
    if not source_files:
        print(f"Error: no .env.enc files found under {args.root_dir}", file=sys.stderr)
        return 1
    failed = False
    processed_files: list[Path] = []
    for source_file in source_files:
        try:
            output_file = source_file.with_name(source_file.name.removesuffix(".enc"))
            decrypt_file(source_file, output_file, args.age_key_file)
...
def _generate_k8s_manifests(args: argparse.Namespace, env_files: list[Path]) -> bool:
    try:
    ...
    referenced_files = {
        env_file for service_files in services.values() for env_file in service_files
    }
    available_files = set(env_files)
    service_groups = []
    failed = False
    for name, service_files in services.items():
        if _report_unavailable_service_files(name, service_files, available_files):
            failed = True
        else:
            service_groups.append((name, service_files))
    manifest_groups = service_groups + [
        (_resource_prefix(args, env_file), [env_file])
        for env_file in env_files
        if env_file not in referenced_files
    ]
    for name, service_files in manifest_groups:
        try:
