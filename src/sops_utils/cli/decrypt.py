...
def parse_arguments() -> argparse.Namespace:
    ...
    input_mode.add_argument(
        "--source-file",
        type=Path,
        help="Decrypt one env file.",
    )
    input_mode.add_argument(
        "--project",
        action="store_true",
        help="Decrypt every *.env.enc file under <root-dir>.",
    )
    parser.add_argument(
        "--output-file",
        type=Path,
        help="Default: <source-file> without .enc (requires --source-file).",
    )
    parser.add_argument(
        "--env-schema-file",
        type=Path,
    ...
    if args.source_file:
        args.output_file = args.output_file or args.source_file.with_name(
            args.source_file.name.removesuffix(".enc")
        )
...
def main() -> None:
    ...
    source_files = (
        [args.source_file]
        if args.source_file
        else find_env_files(args.root_dir, encrypted=True)
    )
    if not source_files:
    ...
    for source_file in source_files:
        output_file = args.output_file or source_file.with_name(
            source_file.name.removesuffix(".enc")
        )
