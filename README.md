...
uv run --directory path/to/sops_utils scripts/encrypt_sops.py --root-dir . --with-k8s
...
Use `--namespace` to set both the Kubernetes namespace and the generated resource
names (`<namespace>-config` and `<namespace>-secrets`). Its default is `riddoc`.
